<?php

namespace App\Http\Controllers\Admin;
use Inertia\Inertia;
use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use App\Http\Requests\Page\StorePageRequest;
use App\Models\Page;
use App\CrudTrait;

class PageController extends Controller
{
    use CrudTrait;


    protected $index_path, $store_rules, $route, $model, $edit_path, $table_name;

    public function __construct(Page $model){
        $this->model = $model;
        $this->route = 'model.index';
        $this->index_path = 'admin/pages/List';
        $this->edit_path = 'admin/pages/EditAdd';
        $this->table_name = 'pages';
        $this->store_rules = StorePageRequest::class;
    }

    public function paginate(Request $request)
    {
        $query = $this->model->query();
        if ($search = $request->input('search')) {
            $query->where('name', 'like', '%' . $search . '%');
        }
        $data = $query->paginate(10);

        return response()->json($data);
    }

    public function create()
    {
        if (!auth()->user()->can('add ' . $this->table_name)) {
            abort(403, 'Unauthorized');
        }
        return Inertia::render($this->edit_path);
    }

   public function store(StorePageRequest $request)
    {
        if (!auth()->user()->can('add ' . $this->table_name)) {
            abort(403, 'Unauthorized');
        }
        try {
            //
        } catch (\Exception $e) {
            return redirect()->back()->with('error', $e->getMessage());
        }
    }

    public function edit(string $id)
    {
        if (!auth()->user()->can('edit ' . $this->table_name)) {
            abort(403, 'Unauthorized');
        }
        $data = $this->model->findOrFail($id);
        return Inertia::render($this->edit_path, ['data' => $data]);
    }

    public function update(StorePageRequest $request)
    {
        if (!auth()->user()->can('edit ' . $this->table_name)) {
            abort(403, 'Unauthorized');
        }
        try {
            //
        } catch (\Exception $e) {
            return redirect()->back()->with('error', $e->getMessage());
        }
    }

}